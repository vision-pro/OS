...
type Tab = "overview" | "content" | "media" | "requests" | "instagram" | "settings";
...
const fieldsByEntity: Record<Entity, Array<{ key: string; label: string; type?: "text" | "textarea" | "number" | "checkbox" | "select"; required?: boolean; options?: Array<[string, string]> }>> = {
  projects: [
    ...
    { key: "categoryId", label: "رقم التصنيف", type: "number" }, { key: "clientName", label: "اسم العميل" }, { key: "projectDate", label: "تاريخ المشروع" },
...
export default function AdminDashboard() {
  ...
  const tabs = [{ id: "overview", label: "نظرة عامة", icon: BarChart3 }, { id: "content", label: "الأعمال والمحتوى", icon: FileText }, { id: "media", label: "الوسائط", icon: Images }, { id: "requests", label: "صندوق الوارد", icon: Mail }, { id: "instagram", label: "Instagram", icon: Video }, { id: "settings", label: "الإعدادات والصلاحيات", icon: Settings2 }] as Array<any>;
  const activeTab = tabs.find(item => item.id === tab);
  return <DashboardLayout><main className="admin-page vision-admin-workspace" dir="rtl"><header className="admin-header admin-command-header"><div className="admin-header-copy"><p className="admin-eyebrow">VISION PRODUCTION / CONTROL ROOM</p><h1>{activeTab?.label || "لوحة إدارة المحتوى"}</h1><p>مسار عمل واحد: أضف الوسائط، اربطها بالمحتوى، ثم انشر واعتمد.</p></div><div className="admin-header-actions"><span className="admin-live-chip"><i />الموقع متصل</span><a href="/" target="_blank" rel="noreferrer" className="admin-public-link">معاينة الموقع <ArrowUpIcon /></a></div></header><nav className="admin-tabs" aria-label="أقسام لوحة التحكم"><span className="admin-tabs-label">مساحة العمل</span><div className="admin-tabs-scroll">{tabs.map(item => <button key={item.id} onClick={() => setTab(item.id)} className={cn(tab === item.id && "active")}><item.icon className="h-4 w-4" /><span>{item.label}</span></button>)}</div></nav>{tab === "overview" && <Overview onChangeTab={setTab} />}{tab === "content" && <ContentManager />}{tab === "requests" && <RequestsManager />}{tab === "media" && <MediaManager />}{tab === "instagram" && <InstagramManager />}{tab === "settings" && <SettingsManager />}</main></DashboardLayout>;
}
...
function ContentEditor({ entity, initial, saving, onClose, onSave }: { entity: Entity; initial: Record<string, any>; saving: boolean; onClose: () => void; onSave: (values: Record<string, unknown>) => void }) {
  ...
  function submit(event: FormEvent) { event.preventDefault(); const clean: Record<string, unknown> = {}; fieldsByEntity[entity].forEach(field => { const value = values[field.key]; if (field.type === "number") clean[field.key] = value === "" ? null : Number(value); else clean[field.key] = value; }); if (entity === "projects") { clean.coverMediaId = coverMediaId; clean.mediaIds = selectedMediaIds; } onSave(clean); }
  return <div className="admin-modal-layer"><form className="admin-modal" onSubmit={submit}><div className="modal-head"><div><p className="admin-eyebrow">{initial.id ? "تعديل" : "إضافة"}</p><h2>{entities.find(item => item.id === entity)?.label}</h2></div><button type="button" onClick={onClose}><X className="h-5 w-5" /></button></div><div className="editor-grid">{fieldsByEntity[entity].map(field => <EditorField key={field.key} field={field} value={values[field.key]} onChange={(value) => setValues(prev => ({ ...prev, [field.key]: value }))} />)}{entity === "projects" ? <ProjectMediaPicker media={media as Record<string, any>[]} loading={mediaLoading} selectedIds={selectedMediaIds} coverId={coverMediaId} onToggle={id => { setSelectedMediaIds(current => { const next = toggleProjectMediaId(current, id); if (!next.includes(id) && coverMediaId === id) setCoverMediaId(null); return next; }); }} onSetCover={id => { setCoverMediaId(id); setSelectedMediaIds(current => current.includes(id) ? current : [...current, id]); }} /> : null}</div><div className="modal-actions"><button type="button" className="admin-ghost" onClick={onClose}>إلغاء</button><button className="admin-primary" disabled={saving}>{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}{initial.id ? "حفظ التعديلات" : "إنشاء العنصر"}</button></div></form></div>;
}

function ProjectMediaPicker({ media, loading, selectedIds, coverId, onToggle, onSetCover }: { media: Record<string, any>[]; loading: boolean; selectedIds: number[]; coverId: number | null; onToggle: (id: number) => void; onSetCover: (id: number) => void }) {
  return <section className="project-media-picker"><div><p className="admin-eyebrow">PROJECT MEDIA</p><h3>اختر فيديو أو صورة للمشروع</h3><p>انقر «إرفاق» لإضافتها إلى المشروع، ثم عيّن واحدة غلافاً. لا تحتاج إلى كتابة أي أرقام.</p></div>{loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <div className="project-media-grid">{media.map(item => { const attached = selectedIds.includes(item.id); const cover = coverId === item.id; return <article className={cn("project-media-choice", attached && "attached", cover && "cover")} key={item.id}>{item.kind === "image" ? <img src={item.url} alt={item.altAr || item.originalName} /> : <div className="media-file"><Video className="h-6 w-6" /><span>{item.kind === "video" ? "VIDEO" : item.kind.toUpperCase()}</span></div>}<div className="project-media-meta"><b>{item.originalName}</b><small>{formatBytes(item.sizeBytes)}</small></div><div className="project-media-actions"><button type="button" className="admin-ghost" onClick={() => onToggle(item.id)}>{attached ? "إزالة" : "إرفاق"}</button><button type="button" className={cn("admin-ghost", cover && "selected-cover")} onClick={() => onSetCover(item.id)}>{cover ? "غلاف المشروع" : "تعيين كغلاف"}</button></div></article>; })}{media.length === 0 ? <div className="admin-empty">ارفع الفيديو أو الصورة أولاً من مكتبة الوسائط.</div> : null}</div>}</section>;
}
