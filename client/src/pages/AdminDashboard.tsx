...
type Tab = "overview" | "content" | "clientLogos" | "requests" | "media" | "instagram" | "users" | "settings";
...
export default function AdminDashboard() {
  ...
  const tabs = [{ id: "overview", label: "نظرة عامة", icon: BarChart3 }, { id: "content", label: "المحتوى", icon: FileText }, { id: "clientLogos", label: "شعارات العملاء", icon: ImagePlus }, { id: "requests", label: "الطلبات", icon: Mail }, { id: "media", label: "الوسائط", icon: Images }, { id: "instagram", label: "فيديوهات Instagram", icon: Video }, { id: "users", label: "المستخدمون", icon: Users }, { id: "settings", label: "الإعدادات", icon: Settings2 }] as Array<any>;
  const activeTab = tabs.find(item => item.id === tab);
  return <DashboardLayout><main className="admin-page vision-admin-workspace" dir="rtl"><header className="admin-header admin-command-header"><div className="admin-header-copy"><p className="admin-eyebrow">VISION PRODUCTION / CONTROL ROOM</p><h1>{activeTab?.label || "لوحة إدارة المحتوى"}</h1><p>إدارة دقيقة للمحتوى والوسائط والنشر من مساحة عمل واحدة.</p></div><div className="admin-header-actions"><span className="admin-live-chip"><i />الموقع متصل</span><a href="/" target="_blank" rel="noreferrer" className="admin-public-link">معاينة الموقع <ArrowUpIcon /></a></div></header><nav className="admin-tabs" aria-label="أقسام لوحة التحكم"><span className="admin-tabs-label">مساحة العمل</span><div className="admin-tabs-scroll">{tabs.map(item => <button key={item.id} onClick={() => setTab(item.id)} className={cn(tab === item.id && "active")}><item.icon className="h-4 w-4" /><span>{item.label}</span></button>)}</div></nav>{tab === "overview" && <Overview onChangeTab={setTab} />}{tab === "content" && <ContentManager />}{tab === "clientLogos" && <ClientLogosManager />}{tab === "requests" && <RequestsManager />}{tab === "media" && <MediaManager />}{tab === "instagram" && <InstagramManager />}{tab === "users" && <UsersManager />}{tab === "settings" && <SettingsManager />}</main></DashboardLayout>;
}
