...
export default function AdminDashboard() {
  ...
  return <DashboardLayout><main className="admin-page" dir="rtl"><header className="admin-header"><div><p className="admin-eyebrow">VISION PRODUCTION / CMS</p><h1>لوحة إدارة المحتوى</h1></div><a href="/" target="_blank" rel="noreferrer" className="admin-public-link">عرض الموقع <ArrowUpIcon /></a></header><div className="admin-tabs">{([{ id: "overview", label: "نظرة عامة", icon: BarChart3 }, { id: "content", label: "المحتوى", icon: FileText }, { id: "clientLogos", label: "شعارات العملاء", icon: ImagePlus }, { id: "requests", label: "الطلبات", icon: Mail }, { id: "media", label: "الوسائط", icon: Images }, { id: "instagram", label: "فيديوهات Instagram", icon: Video }, { id: "users", label: "المستخدمون", icon: Users }, { id: "settings", label: "الإعدادات", icon: Settings2 }] as Array<any>).map(item => <button key={item.id} onClick={() => setTab(item.id)} className={cn(tab === item.id && "active")}><item.icon className="h-4 w-4" />{item.label}</button>)}</div>{tab === "overview" && <Overview onChangeTab={setTab} />}{tab === "content" && <ContentManager />}{tab === "clientLogos" && <ClientLogosManager />}{tab === "requests" && <RequestsManager />}{tab === "media" && <MediaManager />}{tab === "instagram" && <InstagramManager />}{tab === "users" && <UsersManager />}{tab === "settings" && <SettingsManager />}</main></DashboardLayout>;
}
